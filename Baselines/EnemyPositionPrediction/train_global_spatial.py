...
class TrainGlobalSpacial(object):
    ...
    def train(model, env, args):
        ...
        if env_return is not None:
            # actions_gt: torch.Size([10, 32, 6, 66, 51])
            (states_S, states_G, actions_gt), require_init, see_n_replays = env_return
            # print("states_S, states_G, actions_gt:", states_S.shape, states_G.shape, actions_gt.shape)
        ...
        while True:
            ...
            # actions的长度是10，每一个元素的大小是torch.Size([32, 6, 3366])

            action_loss = 0
            for action, action_gt in zip(actions, actions_gt):
                # action:torch.Size([32, 6, 3366])，action_gt:torch.Size([32, 6, 66, 51])
                # print(action_gt.view(args.n_replays, 6, -1).shape, action.view(args.n_replays, 6, -1).shape)
                action_loss += (torch.sum((-action_gt.view(args.n_replays, 6, -1)
                                           * (1e-6 + action.view(args.n_replays, 6, -1)).log()), 2) * (
                                        1 - see_n_replays)).mean()
    ...
    def test(model, env, args):
        ...
        while True:
            ...
            actions_np = np.swapaxes(
                np.asarray([np.argmax(action.data.cpu().numpy().reshape(args.n_replays, 6, -1), axis=2)
                            for action in actions]), 0, 1).squeeze(0)
            actions_gt_np = np.swapaxes(
                np.asarray([np.argmax(action_gt.cpu().numpy().reshape(args.n_replays, 6, -1), axis=2)
        ...
        while True:
            ...
            if env.step_count() % 5 == 0:
                if not see_n_piece[0]:
                    ...
                    plot_predict_heatmap(predict_data, true_pos, env.frame_size, save_pic_dir, str(env.step_count()),
                                         blend_level=.9)
